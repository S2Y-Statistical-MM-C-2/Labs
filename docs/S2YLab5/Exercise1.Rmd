...
#### Now with OLS {-}
(c+) Fit a simple linear regression model with the response variable `Velocity` and the explanatory variable `Distance`
and provide parameter estimates. Fit using OLS (vector-matrix notation). 
...
#### Now with LSE {-}
(c+) Fit a simple linear regression model with the response variable `Velocity` and the explanatory variable `Distance` but with no intercept. Fit using OLS (vector-matrix notation). 
