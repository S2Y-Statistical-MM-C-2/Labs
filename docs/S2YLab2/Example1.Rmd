...
The value of this test statistic is \(1.5358 − 1.7601 = −0.2243\). 
...
The critical value is the $t$ value that corresponds to the significance level of 0.05, and hence is the top limit of our rejection region.
...
```{r}
(Means[1] - Means[2]) / sqrt(Vars[1]/30 + Vars[2]/30)
```


To find the corresponding $p$-value, we could use:

```{r}
pt(-2.518, df=54.7888)
```

In fact, in R the Welch test is automatically chosen within the `t.test()` function:
...
We can check that the observed test statistic (`t = -2.5128`), the degrees of freedom in the $t$-distribution (`df = 54.789`) and the $p$-value (`p-value = 0.007475`) all match our previous calculations. 
...
III. From the R output, 0 does not lie in the 95% confidence interval and thus we reject \(H_0\).
