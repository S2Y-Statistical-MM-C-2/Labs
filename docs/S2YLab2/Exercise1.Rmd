...
```{r}
Diff <- HUBBLE$team1 - HUBBLE$team2
t.obs <- mean(Diff) / (sd(Diff)/sqrt(length(Diff)))
t.obs
p.value <- 2*pt(t.obs, df=length(Diff)-1)
p.value
```
...
```{r}
t.obs <- (mean(HUBBLE$team1)-3)/(sd(HUBBLE$team1)/sqrt(length(HUBBLE$team1)))
t.obs
p.value <- pt(t.obs, df=length(HUBBLE$team1)-1)
p.value
```
