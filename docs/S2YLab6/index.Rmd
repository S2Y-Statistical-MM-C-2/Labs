...
\begin{equation}
\left( \hat{Y}_h - t_{1-\alpha/2;n-p} \cdot s_{\hat{Y}_k},\ \hat{Y}_h + t_{1-\alpha/2;n-p} \cdot s_{\hat{Y}_k}\right). (\#eq:CI_mean)
\end{equation}

$$\small \left( \hat{Y}_h - t_{1-\alpha/2;n-p} \cdot \sqrt{MSE\cdot \mathbf{X}_h (\mathbf{X}_h' \mathbf{X}_h)^{-1}\mathbf{X}_h')},\ \hat{Y}_h + t_{1-\alpha/2;n-p} \cdot \sqrt{MSE\cdot \mathbf{X}_h (\mathbf{X}_h' \mathbf{X}_h)^{-1}\mathbf{X}_h')}\right)$$

The prediction interval for a single new observation is given by
\begin{equation}
\left( \hat{Y}_h - t_{1-\alpha/2;n-p} \cdot s_{\hat{Y}_k},\ \hat{Y}_h + t_{1-\alpha/2;n-p} \cdot s_{\hat{Y}_k}\right). (\#eq:PI)
\end{equation}

$$\small \left( \hat{Y}_h - t_{1-\alpha/2;n-p} \cdot \sqrt{MSE\cdot (1+\mathbf{X}_h (\mathbf{X}_h' \mathbf{X}_h)^{-1}\mathbf{X}_h'))},\ \hat{Y}_h + t_{1-\alpha/2;n-p} \cdot \sqrt{MSE\cdot (1+\mathbf{X}_h (\mathbf{X}_h' \mathbf{X}_h)^{-1}\mathbf{X}_h'))}\right)$$
